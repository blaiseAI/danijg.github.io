// Override original source file part of gatsby-theme-chronoblog
// The main reason is to avoid the `stripWWW` when calling normalize-url
// since DotNetCurry links are broken without the `www.` prefix
...
const ItemLink = ({ item, children, ...props }) => {
  ...
  if (item.frontmatter.link && item.fields.type === 'links') {
    const link = normalizeUrl(item.frontmatter.link, {stripWWW: false});
    return (
      <a
        className="hover-on"
